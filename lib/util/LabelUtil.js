...
function getOptiomalLabelPosition(label, newWaypoints, oldWaypoints, hints) {
  ...
  //
  //     var distance = GeometricUtil.getDistancePointPoint(first, second);
  //
  //     newLabelLineIndex = first;
  //     newLabelLine = [ first, second ];
  //
  //     var newFoot = GeometricUtil.perpendicularFoot(labelPosition, newLabelLine);
  //
  //     if (distance === oldLabelLineDistance) {
  //       x = newFoot.x - oldFoot.x;
  //       y = newFoot.y - oldFoot.y;
  //     }
  //   }
  ...
  // 3. find difference
  var oldWaypointsOriginal = oldWaypoints.map(function(w) { return w.original || w; });
  var newWaypointsOriginal = newWaypoints.map(function(w) { return w.original || w; });

  var diff = findWaypointsDifference(newWaypointsOriginal, oldWaypointsOriginal);
  ...
  if (diff.length == 1) {
    ...
    if (diff.index - 1 == oldLabelLineIndex) {

      newLabelLine = [ newWaypointsOriginal[diff.index-1], newWaypointsOriginal[diff.index] ];
  ...
  if (diff.length == 2) {
    ...
    newLabelLine = [ newWaypointsOriginal[diff.index], newWaypointsOriginal[diff.index+1] ];
  ...
  // segment move with new segments
  if (diff.length == 3 || diff.length == 4) {
